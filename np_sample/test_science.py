...
from dao.file_dao import CsvDAO as cd
...
class TestScience:
    ...
    @staticmethod
    def func_test():
        d_date, d_open, d_high, d_low, d_close = cd.read_test_data_from_csv()
        std = np.std(d_close)
    ...
    @staticmethod
    def test_ave():
        d_date, d_open, d_high, d_low, d_close = cd.read_test_data_from_csv_week_day()
        avg = np.zeros(5)
