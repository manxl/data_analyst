...
@app.route('/ml')
def test_graph122():
    df = pd.read_sql_query("select * from stock_balancesheet where ts_code = '601318.SH' and m = 12;", pool.get_engine())

    mp.plot(df['end_date'], df['total_cur_assets'], label='123')
    # mp.savefig('D:/storage/workspaces/idea-2020/data_analyst/web/static/images/new_plot.png')
    return render_template('abc.html', name=df, url='static/images/new_plot.png')
